...
class CommonByte:
    ...
    def ingest(self, bytes):
        ...
        try:
            self.bytes.update([cert.extensions.get_extension_for_oid(oid.OCSPExtensionOID).public_bytes(serialization.Encoding.DER)])
        ...
        for x in extensions:
            try:
                self.bytes.update([cert.extensions.get_extension_for_class(x).value.public_bytes()])
            except ExtensionNotFound:
                continue
...
def cert_redactor(der_encoding):
    ...
    builder = builder.subject_name(x509.Name([
        x509.NameAttribute(oid.NameOID.COMMON_NAME, u'a'),
    ]))
    ...
    for xtn in cert.extensions:
        if xtn.oid in [oid.ExtensionOID.SUBJECT_KEY_IDENTIFIER,
                    oid.ExtensionOID.SUBJECT_ALTERNATIVE_NAME,
                    ]:
            continue
        builder = builder.add_extension(xtn.value,
            critical=xtn.critical
        )
