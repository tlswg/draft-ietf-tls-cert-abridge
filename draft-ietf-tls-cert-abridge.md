...
If the compressed certificate chain cannot be parsed (e.g. due to incorrect length fields) the decompression algorithm SHOULD return the sentinel value `0xff`. Note that the connection will fail regardless even if this step is not taken as neither certificate validation nor transcript validation can succeed.
